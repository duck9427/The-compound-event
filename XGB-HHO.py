...
def load_npp():
    ...
    data_csv = pd.read_csv(r'NMHW.csv')
    npp = Bunch()
...
def error_rate(X_train, y_train, X_test, y_test, x, opts):
    if abs(x[0]) > 0:
        n_estimators = int(abs(x[0])) + 100
    else:
        n_estimators = int(abs(x[0])) + 100

    if abs(x[1]) > 0:
        learning_rate = (int(abs(x[1])) + 1) / 10
    else:
        learning_rate = (int(abs(x[1])) + 1) / 10

    xgbr_model = XGBRegressor(n_estimators=n_estimators,
                               learning_rate=learning_rate, use_label_encoder=False, eval_metric=['logloss']).fit(
        X_train, y_train)
...
def jfs(X_train, y_train, X_test, y_test, opts):
    ...
    while t < max_iter:
        ...
        print("Optimal Fitness Value: ", curve[0, t])
        t += 1
...
#   X = df.drop(['NPP', 'lat', 'lon'], axis=1)
    ...
    if abs(fmdl[0][0]) > 0:
        best_n_estimators = int(abs(fmdl[0][0])) + 1000
    else:
        best_n_estimators = int(abs(fmdl[0][0])) + 1500

    if abs(fmdl[0][1]) > 0:
        best_learning_rate = (int(abs(fmdl[0][1])) + 1) / 100
    else:
        best_learning_rate = (int(abs(fmdl[0][1])) + 1) / 100

    print('----------------HXGB Regression-Optimum Results-----------------')
    ...
    accuracies = cross_val_score(xgbr_model, X=X_train, y=y_train, cv=10)
    accuracy_mean = accuracies.mean()

    print('----------------Model Evaluation-----------------')
    print('XGBoost Regression-Best Parameter-R2 Train:', evaluate_model(xgbr_model, X_train, y_train))
    print('XGBoost Regression-Best Parameter-R2 Test:', evaluate_model(xgbr_model, X_test, y_test))
    print('XGBoost Regression-Best Parameter-R2 Train: {}'.format(round(xgbr_model.score(X_train, y_train), 5)))
